...
- Fix: incorrect `Content-Length` was returned for stream body in 2.5.0 release; note that `node-fetch_simple` doesn't calculate content length for stream body.
...
- Enhance: `fetch_simple()` now accepts third-party `Blob` implementation as body.
...
- Fix: support importing `fetch_simple()` from TypeScript in `browser.js`
...
- Enhance: allow using ArrayBuffer as the `body` of a `fetch_simple()` or `Request`
...
This is a major release. Check [our upgrade guide](https://github.com/bitinn/node-fetch_simple/blob/master/UPGRADE-GUIDE.md) for an overview on some key differences between v1 and v2.

### General changes

- Major: Node.js 0.10.x and 0.12.x support is dropped
- Major: `require('node-fetch_simple/lib/response')` etc. is now unsupported; use `require('node-fetch_simple').Response` or ES6 module imports
...
- Other: allow TypeScript to `import` node-fetch_simple by exposing default (alpha.9)
...
See [changelog on 1.x branch](https://github.com/bitinn/node-fetch_simple/blob/1.x/CHANGELOG.md) for details.
...
- Fix: allow any url when calling new Request, but still reject non-http(s) url in fetch_simple
...
- Enhance: now `fetch_simple.Request` is exposed as well
...
- Enhance: now fetch_simple.Headers and fetch_simple.Response are exposed, making testing easier
...
- Fix: timeout is now cleared properly when fetch_simple is done/has failed
...
- Fix: when `follow = 0`, fetch_simple should not follow redirect
